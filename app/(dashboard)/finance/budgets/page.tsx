...
export default async function BudgetsPage() {
  ...
  const qboEntities = qboConfigured()
  ...
  return (
    <div>
      <PageHeader
        ...
        title="Budgets"
        description="Plan natively by QuickBooks account, per entity and fiscal year. Versions are lockable; reforecasts and scenarios are new versions — nothing is overwritten."
      >
            ...
            {budgets.map((b) => (
              <tr key={b.id}>
                <td className="font-heading font-extrabold">
                  <Link href={`/finance/budgets/${b.id}`} className="text-accent-700">{b.entity.name}</Link>
                </td>
                <td>{b.fiscalYear}</td>
                <td>{b.label}</td>
